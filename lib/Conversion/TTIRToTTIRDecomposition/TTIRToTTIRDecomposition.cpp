...
struct DotGeneralToMatmulConversionPattern
  ...
  matchAndRewrite(ttir::DotGeneralOp op, OpAdaptor adaptor,
    ...
    // Check if the original operation should be hoisted
    ...
    // Propagate the should_hoist attribute to the matmul operation
    if (shouldHoist) {
    ...
    // Propagate the should_hoist attribute to the final reshape operation
    if (shouldHoist) {
...
private:
  ...
  ttir::PermuteOp createPermuteOp(PatternRewriter &rewriter, Location loc,
    ...
    // Propagate the should_hoist attribute if needed
    if (shouldHoist) {
